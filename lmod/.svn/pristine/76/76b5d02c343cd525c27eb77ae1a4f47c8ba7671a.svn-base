...
sed -i 's/bash)\ \.\ \/cvos\/local\/apps\/environment-modules\/3\.2\.6\/\/Modules\/default\/init\/bash/bash)\ \.\ \/cvos\/local\/apps\/environment-modules\/3\.2\.6\/\/Modules\/default\/init\/bash;\ export\ -f\ module/g' %{buildroot}/etc/profile.d/modules.sh
sed -i 's/sh)\ \.\ \/cvos\/local\/apps\/environment-modules\/3\.2\.6\/\/Modules\/default\/init\/sh/sh)\ \.\ \/cvos\/local\/apps\/environment-modules\/3\.2\.6\/\/Modules\/default\/init\/sh;\ export\ -f\ module/g' %{buildroot}/etc/profile.d/modules.sh
...
rm -f %{buildroot}/cvos/local/modulefiles/modules
...
%post
if ! grep -q ^module /etc/skel/.bashrc; then echo "module load gcc" >> /etc/skel/.bashrc; fi
